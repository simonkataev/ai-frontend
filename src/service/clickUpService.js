...
export default class ClickUpService {
  ...
  getSpaces = async (teamId) => {
    const response = await axios.get(
      this.endpoint + "team/" + teamId + "/space",
      this.config,
    );
  ...
  getFolders = async (spaceId) => {
    const response = await axios.get(
      this.endpoint + "space/" + spaceId + "/folder",
      this.config,
    );
  ...
  getFolderlessLists = async (spaceId) => {
    const response = await axios.get(
      this.endpoint + "space/" + spaceId + "/list",
      this.config,
    );
  ...
  getListsFromFolder = async (folderId) => {
    const response = await axios.get(
      this.endpoint + "folder/" + folderId + "/list",
      this.config,
    );
  ...
  getTasks = async (listId) => {
    const response = await axios.get(
      this.endpoint + "list/" + listId + "/task",
      this.config,
    );
